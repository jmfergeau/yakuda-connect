...
def test_xr_hotas_in_tools_json_is_installable():
    import programs
    tool = next(t for t in programs.all_tools() if t["key"] == "xr-hotas")
    assert appimg.detect_install_methods(tool) == ["cargo"]
    assert ci.git_url(tool) == "https://github.com/galister/xr-hotas.git"
    assert "libopenxr-dev" in ci.sys_deps(tool)["debian"]
